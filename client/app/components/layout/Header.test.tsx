...
let mockUnreadCountData:
  | { count: number; byType: object; byPriority: object }
  | undefined = { count: 2, byType: {}, byPriority: {} };
...
describe("Header", () => {
  ...
  describe("rendering", () => {
    ...
    it("should render mobile menu toggle button", () => {
      ...
      expect(
        screen.getByLabelText(/cerrar menu|abrir menu/i),
      ).toBeInTheDocument();
    });
  ...
  describe("search functionality", () => {
    ...
    it("should close search when clear button is clicked", async () => {
      ...
      const searchCloseButton = closeButtons.find((btn) =>
  ...
  describe("profile dropdown", () => {
    ...
    it("should have profile link", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByRole("link", { name: /mi perfil/i }),
        ).toBeInTheDocument();
      });
    });
  ...
  describe("spotlight search overlay rendering", () => {
    ...
    it("should close search when clicking on spotlight overlay backdrop", async () => {
      ...
      if (overlay) {
        ...
        await waitFor(() => {
          expect(
            document.querySelector(".fixed.inset-0.z-50.bg-neutral-900\\/60"),
          ).not.toBeInTheDocument();
        });
      }
    });
  ...
  describe("mark all as read pending state", () => {
    it("should show loading spinner when mark all as read is pending (line 304)", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Marcar todas como leidas"),
        ).toBeInTheDocument();
      });
      ...
      const markAllButton = screen
        .getByText("Marcar todas como leidas")
        .closest("button");
  ...
  describe("notification icon/style for error category (lines 144, 157)", () => {
    it("should render error notification with correct icon and styling", async () => {
      ...
      const errorNotificationItem = screen
        .getByText("Pago fallido")
        .closest('div[class*="cursor-pointer"]');
      ...
      const iconContainer = errorNotificationItem?.querySelector(
        '[class*="bg-error"]',
      );
  ...
  describe("edge cases for branch coverage", () => {
    it("should show role as-is when role is unknown", async () => {
      ...
      // Set an unknown role
      useAuthStore.setState({
        user: { ...mockUser, role: "UNKNOWN_ROLE" as never },
      });
    ...
    it("should show empty state when no notifications exist (line 327 else branch)", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("No tienes notificaciones"),
        ).toBeInTheDocument();
      });

      // The "View all notifications" link should not be visible when there are no notifications
      expect(
        screen.queryByText("Ver todas las notificaciones"),
      ).not.toBeInTheDocument();
    });
  });
});
