...
import type { Route } from "./+types/_app.accounting.reports.ar-aging";
import { PageWrapper, PageSection } from "~/components/layout/PageWrapper";
import { formatCurrency } from "~/lib/utils";
import { useARAgingReport } from "~/hooks/useAccountingReports";
...
export const meta: Route.MetaFunction = () => {
  return [
    { title: "Cartera CxC - StockFlow" },
    { name: "description", content: "Informe de cartera por edades - Cuentas por Cobrar" },
  ];
};
...
export default function ARAgingPage() {
  ...
  const { data, isLoading, isError } = useARAgingReport(asOfDate);
    ...
    <PageWrapper>
      <PageSection className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-xl bg-warning-50 text-warning-500 dark:bg-warning-900/20">
            <Clock className="h-5 w-5" />
          </div>
          <div>
            <h1 className="text-2xl font-bold font-display text-neutral-900 dark:text-white">
              Cartera CxC por Edades
            </h1>
            <p className="text-sm text-neutral-500 dark:text-neutral-400">
              Facturas pendientes agrupadas por cliente y antiguedad
            </p>
          </div>
        </div>
      </PageSection>
            ...
            <div className="flex items-center justify-between">
              ...
              {data && (
                <span className="text-sm font-medium text-neutral-500">
                  {data.rows.length} cliente{data.rows.length !== 1 ? "s" : ""}
                </span>
              )}
            </div>
          </CardHeader>
          <CardContent className="p-0">
            {isLoading ? (
              ...
              <EmptyState
                ...
                title="Sin datos"
                description="No se encontraron facturas pendientes para la fecha seleccionada."
              />
            ) : (
