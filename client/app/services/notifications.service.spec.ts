...
describe('notificationsService', () => {
  ...
  describe('getNotification', () => {
    ...
    it('should handle api errors', async () => {
      ...
      await expect(notificationsService.getNotification('invalid-id')).rejects.toThrow('Not found');
    });
  });
  ...
  describe('getRecentNotifications', () => {
    ...
    it('should return array of notifications', async () => {
      vi.mocked(api.get).mockResolvedValue({ data: [mockNotification, mockNotificationRead] });
  ...
  describe('markAsRead', () => {
    it('should call api.patch with correct endpoint', async () => {
      const readNotification = { ...mockNotification, read: true, readAt: new Date().toISOString() };
    ...
    it('should handle api errors', async () => {
      ...
      await expect(notificationsService.markAsRead('invalid-id')).rejects.toThrow('Not found');
    });
  });
  ...
  describe('markMultipleAsRead', () => {
    ...
    it('should handle empty array', async () => {
      vi.mocked(api.patch).mockResolvedValue({ data: { success: true, updatedCount: 0 } });
  ...
  describe('markAllAsRead', () => {
    it('should call api.patch with correct endpoint', async () => {
      vi.mocked(api.patch).mockResolvedValue({ data: { success: true, updatedCount: 5 } });
  ...
  describe('markAsUnread', () => {
    it('should call api.patch with correct endpoint', async () => {
      const unreadNotification = { ...mockNotification, read: false, readAt: undefined };
  ...
  describe('deleteNotification', () => {
    ...
    it('should handle api errors', async () => {
      ...
      await expect(notificationsService.deleteNotification('invalid-id')).rejects.toThrow('Not found');
    });
  });
  ...
  describe('deleteMultipleNotifications', () => {
    it('should call api.delete with correct endpoint and body', async () => {
      ...
      const ids = ['notif-1', 'notif-2'];
      const result = await notificationsService.deleteMultipleNotifications(ids);

      expect(api.delete).toHaveBeenCalledWith('/notifications', { data: { ids } });
  ...
  describe('createNotification', () => {
    it('should call api.post with correct endpoint and body', async () => {
      ...
      vi.mocked(api.post).mockResolvedValue({ data: { ...mockNotification, ...newNotification } });

      const result = await notificationsService.createNotification(newNotification);
  ...
  describe('edge cases', () => {
    it('should handle network timeout', async () => {
      ...
      await expect(notificationsService.getNotifications()).rejects.toThrow('timeout');
    });
    ...
    it('should handle 401 unauthorized', async () => {
      ...
      await expect(notificationsService.getNotifications()).rejects.toEqual(error);
    });
    ...
    it('should handle 500 server error', async () => {
      ...
      await expect(notificationsService.getUnreadCount()).rejects.toEqual(error);
    });

    it('should handle empty response data', async () => {
      vi.mocked(api.get).mockResolvedValue({ data: { data: [], meta: { total: 0, page: 1, limit: 10, totalPages: 0, unreadCount: 0 } } });
  ...
  describe('filter combinations', () => {
    it('should handle all filters combined', async () => {
      ...
      const expectedParams = 'page=1&limit=10&type=LOW_STOCK&priority=HIGH&read=false&search=test&sortBy=createdAt&sortOrder=desc&startDate=2024-01-01&endDate=2024-12-31';
