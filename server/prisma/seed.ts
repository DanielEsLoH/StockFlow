...
async function main() {
  ...
  // ============================================================================
  // STEP 20: Payroll (Config + Employees + Periods + Entries)
  // ============================================================================
  ...
  const empCarlos = await prisma.employee.create({
    data: {
      ...
      firstName: 'Carlos',
      lastName: 'Mendoza',
      email: 'carlos.mendoza@tienda-demo.com',
  ...
  const empMaria = await prisma.employee.create({
    data: {
      ...
      firstName: 'María',
      lastName: 'López',
      email: 'maria.lopez@tienda-demo.com',
      phone: '+57 310 222 3333',
      address: 'Carrera 7 #80-15',
      city: 'Bogotá',
      cityCode: '11001',
      department: 'Bogotá D.C.',
      departmentCode: '11',
  ...
  const empJuan = await prisma.employee.create({
    data: {
      tenantId: tenantDemo.id,
      documentType: 'CC',
      documentNumber: '1055443322',
      firstName: 'Juan',
      lastName: 'Rodríguez',
      email: 'juan.rodriguez@tienda-demo.com',
  ...
  const empAna = await prisma.employee.create({
    data: {
      tenantId: tenantDemo.id,
      documentType: 'CC',
      documentNumber: '1033221100',
      firstName: 'Ana',
      lastName: 'Torres',
      email: 'ana.torres@tienda-demo.com',
  ...
  const empPedro = await prisma.employee.create({
    data: {
      tenantId: tenantDemo.id,
      documentType: 'CC',
      ...
      firstName: 'Pedro',
      lastName: 'Gómez',
      email: 'pedro.gomez@tienda-demo.com',
      phone: '+57 301 888 9999',
      address: 'Carrera 30 #50-10',
      city: 'Bogotá',
      cityCode: '11001',
      department: 'Bogotá D.C.',
      departmentCode: '11',
      contractType: 'OBRA_O_LABOR',
      salaryType: 'ORDINARIO',
      baseSalary: 1800000,
      auxilioTransporte: true,
  ...
  await prisma.employee.create({
    data: {
      tenantId: tenantDemo.id,
      documentType: 'CC',
      documentNumber: '1066554433',
      firstName: 'Laura',
      lastName: 'Díaz',
      email: 'laura.diaz@tienda-demo.com',
      phone: '+57 318 000 1111',
      address: 'Calle 72 #10-05',
      city: 'Bogotá',
      cityCode: '11001',
      department: 'Bogotá D.C.',
      departmentCode: '11',
  ...
  const empAndres = await prisma.employee.create({
    data: {
      tenantId: tenantDistribuidora.id,
      documentType: 'CC',
      documentNumber: '1044332211',
      firstName: 'Andrés',
      lastName: 'Martínez',
      email: 'andres.martinez@distribuidoranacional.com',
  ...
  const empSofia = await prisma.employee.create({
    data: {
      ...
      firstName: 'Sofía',
      lastName: 'Ramírez',
      email: 'sofia.ramirez@distribuidoranacional.com',
  ...
  // ── PayrollPeriods ──
  // Demo: Dic 2025 (CLOSED), Ene 2026 (APPROVED), Feb 2026 (OPEN)
  const periodDemoDic = await prisma.payrollPeriod.create({
    data: {
      ...
      status: 'CLOSED',
      employeeCount: 4,
      totalDevengados: 25723500,
      totalDeducciones: 2213480,
      totalNeto: 23510020,
    },
  });
  ...
  const periodDnEne = await prisma.payrollPeriod.create({
    data: {
      ...
      status: 'CALCULATED',
      employeeCount: 2,
      totalDevengados: 6200000,
      totalDeducciones: 492000,
      totalNeto: 5708000,
    },
  });
  ...
  // ── PayrollEntries ──
  // Helper: cálculos colombianos reales
  // Carlos: SMMLV 1,423,500 + auxilio 200,000 = 1,623,500 devengados
  //   Salud 4% = 56,940 | Pensión 4% = 56,940 | Total ded = 113,880 | Neto = 1,509,620
  //   Empleador: Salud 8.5%=120,998 Pensión 12%=170,820 ARL 0.522%=7,431 Caja 4%=56,940 SENA 2%=28,470 ICBF 3%=42,705
  const createEntry = async (
    ...
    await prisma.payrollEntry.create({
      data: {
        ...
        viaticos: data.viaticos ?? 0,
        incapacidad: data.incapacidad ?? 0,
        licencia: data.licencia ?? 0,
        vacaciones: data.vacaciones ?? 0,
        otrosDevengados: data.otrosDevengados ?? 0,
        totalDevengados: data.totalDevengados,
        saludEmpleado: data.saludEmpleado,
        pensionEmpleado: data.pensionEmpleado,
        fondoSolidaridad: data.fondoSolidaridad ?? 0,
        retencionFuente: data.retencionFuente ?? 0,
        sindicato: data.sindicato ?? 0,
        libranzas: data.libranzas ?? 0,
        otrasDeducciones: data.otrasDeducciones ?? 0,
        totalDeducciones: data.totalDeducciones,
        saludEmpleador: data.saludEmpleador ?? 0,
        pensionEmpleador: data.pensionEmpleador ?? 0,
        arlEmpleador: data.arlEmpleador ?? 0,
        cajaEmpleador: data.cajaEmpleador ?? 0,
        senaEmpleador: data.senaEmpleador ?? 0,
        icbfEmpleador: data.icbfEmpleador ?? 0,
        provisionPrima: data.provisionPrima ?? 0,
        provisionCesantias: data.provisionCesantias ?? 0,
        provisionIntereses: data.provisionIntereses ?? 0,
        provisionVacaciones: data.provisionVacaciones ?? 0,
        totalNeto: data.totalNeto,
      },
    });
  };

  // Carlos Mendoza: SMMLV con auxilio transporte
  // IBC = sueldo 1,423,500 (sin auxilio) → salud/pensión sobre IBC
  const carlosData = {
  ...
  // María López: 3,500,000 sin auxilio (>2 SMMLV)
  // IBC = 3,500,000
  const mariaData = {
  ...
  // Juan Rodríguez: Integral 18,500,000 — IBC = 70% = 12,950,000
  // Fondo solidaridad: 1% sobre IBC >4 SMMLV
  const juanData = {
  ...
  // Ana Torres: 2,000,000 + auxilio, ARL II (1.044%)
  // IBC = 2,000,000
  const anaData = {
  ...
  // Dic 2025 entries (CLOSED → ACCEPTED status)
  let entryCounter = 1;
  for (const [empId, data] of [
    [empCarlos.id, carlosData],
    [empMaria.id, mariaData],
    [empJuan.id, juanData],
    [empAna.id, anaData],
  ] as [string, Record<string, any>][]) {
    await createEntry(tenantDemo.id, periodDemoDic.id, empId, `NE-${String(entryCounter).padStart(4, '0')}`, 'ACCEPTED', data);
    entryCounter++;
  }
  ...
  for (const [empId, data] of [
    ...
    await createEntry(tenantDemo.id, periodDemoEne.id, empId, `NE-${String(entryCounter).padStart(4, '0')}`, 'APPROVED', data);
    entryCounter++;
  }

  // Distribuidora: Ene 2026 (CALCULATED)
  // Andrés: 1,800,000 + auxilio, ARL II
  const andresData = {
  ...
  // Sofía: 4,200,000 sin auxilio, ARL I
  const sofiaData = {
  ...
  await createEntry(tenantDistribuidora.id, periodDnEne.id, empAndres.id, 'NE-0001', 'CALCULATED', andresData);
  await createEntry(tenantDistribuidora.id, periodDnEne.id, empSofia.id, 'NE-0002', 'CALCULATED', sofiaData);

  const payrollEntryCount = 10; // 4 + 4 + 2
  console.log(`   ✅ ${payrollEntryCount} Payroll Entries created`);
  ...
  console.log(`  Payroll Configs:   2`);
  console.log(`  Employees:         8 (6 Demo + 2 Distribuidora)`);
  console.log(`  Payroll Periods:   4 (3 Demo + 1 Distribuidora)`);
